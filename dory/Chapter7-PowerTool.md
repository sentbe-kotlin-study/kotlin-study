...
- 프로퍼티가 val인 경우 위임 객체의 클래스에는 getValue()가, var인 경우엔 getValue(), setValue() 함수 정의가 있어야 한다.

### 85. 프로퍼티 위임 도구
<<<<<<< HEAD
- 표준 라이브러리에는 특별한 프로퍼티 위임 연산이 들어있다.
- Map은 위임 프로퍼티의 위임 객체로 쓰일 수 있도록 미리 설정ㅈ된 코틀린 표준 라이브러리 타입 중 하나다.

### 86. 지연 계산 초기화
- 프로퍼티 초기화
  - 프로퍼티를 정의하는 시점이나 생성자 안에서 초깃값을 저장한다.
  - 프로퍼티에 접근할 때 마다 값을 계산하는 커스텀 게터를 정의한다.
