...
- 식과 문의 차이점 
  - 문(statement)은 효과를 발생시키지만 결과를 내놓지 않는다. -> 주변 상태를 변경해야 한다. 
  - 식(expression)은 항상 결과를 만들어낸다. 이 값을 변수에 대입하거나 다른 식의 일부분으로 사용이 가능하다.

> 예시

```kotlin
fun main() {
    // for문 자체는 아무런 결과값도 만들어내지 못한다. -> for 루프는 '문'이다
    val f = for(i in 1..0) {} 
  
    // if는 식을 만든다. if의 결과를 변수에 대입할 수 있다.
    val a = if (11 > 42) 9 else 5
}
```
